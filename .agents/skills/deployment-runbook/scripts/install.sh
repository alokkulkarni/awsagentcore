#!/usr/bin/env bash
# Installs the deployment-runbook skill to various AI IDE / CLI tool locations.
# Usage: ./install.sh [--project [PATH]] [--global] [--kiro [PATH]] [--claude [PATH]] [--copilot [PATH]] [--all]

set -euo pipefail

SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SKILL_NAME="deployment-runbook"
BEGIN_MARKER="<!-- BEGIN deployment-runbook skill -->"
END_MARKER="<!-- END deployment-runbook skill -->"
