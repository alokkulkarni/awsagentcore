#!/usr/bin/env bash
# Installs the github-workflow-automation skill to various locations
# Usage: ./install.sh [--project [PATH]] [--global] [--kiro [PATH]] [--claude [PATH]] [--all] [--copilot [PATH]]

set -euo pipefail

SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SKILL_NAME="github-workflow-automation"
BEGIN_MARKER="<!-- BEGIN github-workflow-automation skill -->"
END_MARKER="<!-- END github-workflow-automation skill -->"
...
install_kiro()    { local base="${1:-$HOME}"; copy_skill "$base/.kiro/skills/$SKILL_NAME"; }
install_claude()  { local base="${1:-$HOME}"; copy_skill "$base/.claude/skills/$SKILL_NAME"; }
install_project() { local base="${1:-$HOME}"; copy_skill "$base/.agents/skills/$SKILL_NAME"; }
...
install_copilot() {
    ...
    mkdir -p "$target_dir"
    local temp_file="$target_dir/.github-workflow-automation-fragment"
