...
SKILL_NAME="sdlc-architecture"
BEGIN_MARKER="<!-- BEGIN sdlc-architecture skill -->"
END_MARKER="<!-- END sdlc-architecture skill -->"

copy_skill() {
    local target="$1"; mkdir -p "$target"
    [ "$(cd "$target" && pwd)" = "$SKILL_DIR" ] && { echo "✓ Already installed: $target"; return; }
    cp -R "$SKILL_DIR/." "$target/"; echo "✓ Installed to: $target"
}
...
install_copilot() {
    local base="${1:-$HOME}"; local tf="$base/.github/copilot-instructions.md"
    mkdir -p "$base/.github"
    local tmp="$base/.github/.${SKILL_NAME}-frag"
...
p=re.compile(r'<!-- BEGIN '+re.escape(s)+r' skill -->.*?<!-- END '+re.escape(s)+r' skill -->\n?',re.DOTALL)
t.write_text(p.sub(f+'\n',text),encoding='utf-8')
PY
        rm -f "$tmp"; echo "✓ Updated: $tf"; return
    fi
    [ -f "$tf" ] && [ -s "$tf" ] && printf '\n' >> "$tf"
    cat "$tmp" >> "$tf"; rm -f "$tmp"; echo "✓ Appended to: $tf"
}
