#!/usr/bin/env bash
# Installs the webapp-scaffold skill to various locations
# Usage: ./install.sh [--project [PATH]] [--global] [--kiro [PATH]] [--claude [PATH]] [--all] [--copilot [PATH]]

set -euo pipefail

SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SKILL_NAME="webapp-scaffold"
BEGIN_MARKER="<!-- BEGIN webapp-scaffold skill -->"
END_MARKER="<!-- END webapp-scaffold skill -->"
...
install_copilot() {
    ...
    mkdir -p "$target_dir"
    local temp_file="$target_dir/.webapp-scaffold-fragment"
    {
        printf '%s\n' "$BEGIN_MARKER"
...
pattern = re.compile(r'<!-- BEGIN webapp-scaffold skill -->.*?<!-- END webapp-scaffold skill -->\n?', re.DOTALL)
