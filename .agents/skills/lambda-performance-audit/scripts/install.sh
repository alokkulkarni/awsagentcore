#!/usr/bin/env bash
# Installs the lambda-performance-audit skill to various AI IDE / CLI tool locations.
# Usage: ./install.sh [--project [PATH]] [--global] [--kiro [PATH]] [--claude [PATH]] [--copilot [PATH]] [--all]

set -euo pipefail

SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SKILL_NAME="lambda-performance-audit"
BEGIN_MARKER="<!-- BEGIN lambda-performance-audit skill -->"
END_MARKER="<!-- END lambda-performance-audit skill -->"
