...
system: |
  ...
  - Sort codes (6 digits, written XX-XX-XX): "six zero dash zero zero dash zero one" — speak each digit including the dashes as "dash": "six zero dash zero zero dash zero one"
  ...
  - Month first for UK: "the third of January" — NOT "January third"
  ...
  1. Call request_channel_transfer(session_id, instance_id, target_channel, customer_phone, reason).
     ...
     - customer_phone: for VOICE→CHAT, use inbound caller ID ({{$.Custom.customerPhone}}) — do NOT ask the customer for it; they are already on the phone. For CHAT→VOICE, use the number they provide.
...
tools:
  ...
  - name: request_channel_transfer
    description: >
      ...
      then call escalate_to_human_agent with escalation_reason='channel_transfer' to exit
      the AI agent block and allow the flow's transfer Lambda to execute.
    input_schema:
      type: object
      properties:
        session_id:
          type: string
          description: The current session identifier (matches ContactId injected by session_injector).
        instance_id:
          type: string
        ...
        target_channel:
          ...
          enum:
            ...
            - voice
          description: >
            'chat' — customer is on VOICE and wants a chat link sent by SMS.
            'voice' — customer is on CHAT and wants an outbound phone callback.
        customer_phone:
          type: string
          description: >
            ...
            For target_channel='chat': use {{$.Custom.customerPhone}} (inbound caller ID) — do NOT ask.
        reason:
          type: string
          description: Brief reason for the transfer in plain English (max 200 chars). Use customer's own words.
      required:
        - session_id
        - instance_id
        - target_channel
