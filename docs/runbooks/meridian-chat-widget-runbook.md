# Meridian Chat Widget Operational Runbook

| Field | Value |
|---|---|
| **Document ID** | RNB-MCW-001 |
| **Companion Playbook** | PLY-MCW-001 |
| **Version** | 1.0 |
...
### 1.3 Confirm Meridian widget source configuration

```bash
cd /Users/alokkulkarni/Documents/Development/awsagentcore
grep -n "snippetId\|customDisplayNames\|contactAttributes\|conversationalbot.my.connect.aws" connect-chat-widget/index.html
grep -n "BOT\|SYSTEM\|ARIA\|Meridian Bank" connect-chat-widget/src/components/ConnectChatWidget.jsx
```

**✓ Verify**

```bash
grep -n "port" connect-chat-widget/vite.config.js
```

**Expected result:** `vite.config.js` shows port `4000`; `index.html` shows the hosted widget bootstrap.
...
- The hosted widget snippet in `connect-chat-widget/index.html` is the intended one.
- Approved origins include `http://localhost:4000` for local testing.
...
```bash
cd /Users/alokkulkarni/Documents/Development/awsagentcore/connect-chat-widget
npm install
npm run dev
```
...
The Meridian widget does **not** use a `.env` file today. Update configuration in:

- `connect-chat-widget/index.html` for hosted widget snippet, styles, contact attributes, and display names
- `connect-chat-widget/src/components/ConnectChatWidget.jsx` for MutationObserver relabeling
- `connect-chat-widget/src/App.jsx` and CSS files for Meridian page content/branding
...
**Expected result:** `dist/index.html`, `dist/assets/*`, and copied public assets such as `dist/favicon.svg` are present.
...
```bash
bash scripts/deploy_connect_widget.sh --env staging
bash scripts/deploy_connect_widget.sh --env prod --region eu-west-2
bash scripts/deploy_connect_widget.sh --bucket-name <your-unique-bucket-name>
bash scripts/deploy_connect_widget.sh --no-build
bash scripts/deploy_connect_widget.sh --dry-run
```
...
In order, `scripts/deploy_connect_widget.sh`:
...
- `Distribution ID`
- `Widget URL`
- `State file` (`connect-chat-widget/.deploy-state-<env>.env`)
...
- Meridian page renders correctly
...
- `connect-chat-widget/src/App.jsx` (if UI text/layout changed)
...
1. Open the deployed Meridian widget URL in Chrome.
...
8. Confirm transcript labels show `ARIA` and `Meridian Bank`.
9. Confirm no browser console errors appear during chat start.

**Helpful checks**

- No `403` for JS/CSS/static assets
...
```bash
export BUCKET_NAME="<meridian-connect-widget-bucket>"
...
- Check browser console for JS load failures.

### Chat will not connect

- Confirm the hosted widget snippet in `connect-chat-widget/index.html` is correct.
