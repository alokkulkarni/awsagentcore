...
const OUTCOME_CONFIG = {
  completed: { color: '#34d399', bg: 'bg-emerald-500/15', text: 'text-emerald-600 dark:text-emerald-400', label: 'Completed',  icon: '✓', desc: 'Reached end of flow (Disconnect block)' },
  escalated: { color: '#fb923c', bg: 'bg-orange-500/15',  text: 'text-orange-600 dark:text-orange-400',  label: 'Escalated',  icon: '↗', desc: 'Transferred to a human agent' },
  abandoned: { color: '#f43f5e', bg: 'bg-rose-500/15',    text: 'text-rose-600 dark:text-rose-400',    label: 'Abandoned',  icon: '✕', desc: 'Contact hung up mid-flow' },
  error:     { color: '#c084fc', bg: 'bg-purple-500/15',  text: 'text-purple-600 dark:text-purple-400',  label: 'Error',      icon: '!', desc: 'Lambda / API error was last recorded event' },
  unknown:   { color: '#64748b', bg: 'bg-slate-700/50',   text: 'text-slate-500 dark:text-slate-400',   label: 'Unknown',    icon: '?', desc: 'Last block was not a recognised terminal type — may have transitioned to a sub-flow whose logs are in a different log group' },
};
...
function OutcomesSummary({ summary, total }) {
  ...
  return (
    <div className="mb-6">
      <p className="text-[10px] text-slate-600 dark:text-slate-500 mb-2 italic">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {keys.map((k) => {
          ...
          return (
            <div
              key={k}
              className={`rounded-xl border border-slate-200 dark:border-slate-800 bg-white/60 dark:bg-slate-900/60 px-4 py-3 flex flex-col gap-1 ${cfg.bg}`}
              title={cfg.desc}
            >
              ...
              <div className="text-[10px] text-slate-600 dark:text-slate-500">{pct}% of classified</div>
...
function FunnelTooltip({ active, payload }) {
  ...
  return (
    <div className="rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2.5 text-xs shadow-xl min-w-[220px]">
      <p className="font-semibold text-slate-800 dark:text-slate-200 mb-2">{d.block_name}</p>
      <div className="space-y-1">
        <p className="text-slate-500 dark:text-slate-400">
          Type: <span className="text-slate-700 dark:text-slate-300">{d.block_type}</span>
        </p>
        {occurrence && (
          <p className="text-[10px] text-slate-600 dark:text-slate-500 italic">
            {occurrence === 1 ? '1st' : occurrence === 2 ? '2nd' : occurrence === 3 ? '3rd' : `${occurrence}th`} occurrence of this block type in the flow
          </p>
        )}
        <p className="text-slate-500 dark:text-slate-400">
          Contacts reached:{' '}
          <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{d.count}</span>
        ...
        {d.drop_count > 0 ? (
          <>
            ...
            <p className="text-slate-500 dark:text-slate-400">
              ...
              <span className="text-cyan-600 dark:text-cyan-400 font-semibold">{nextCount}</span>
            </p>
            <p className="text-slate-500 dark:text-slate-400">
              ...
              {d.drop_pct > DROP_MID && (
                <span className="ml-1.5 rounded-full bg-red-500/15 px-1.5 py-0.5 text-[9px] font-medium text-red-600 dark:text-red-400">
                  HIGH
                </span>
              )}
              {d.drop_pct > DROP_LOW && d.drop_pct <= DROP_MID && (
                <span className="ml-1.5 rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[9px] font-medium text-amber-600 dark:text-amber-400">
                  MODERATE
                </span>
              )}
            </p>
            {d.exit_intent && (
              <div className="mt-1.5 flex items-center gap-1.5">
                <span className="text-slate-600 dark:text-slate-500 text-[10px]">Exit type:</span>
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-5 text-xs text-slate-500 dark:text-slate-400">
        <span>
          <span className="text-slate-800 dark:text-slate-200 font-semibold">{total}</span> total contacts over{' '}
          <span className="text-slate-800 dark:text-slate-200 font-semibold">{data.period_days}</span> days
        </span>
        ...
        <span className="ml-auto text-[10px] text-slate-600 italic">
          Numbers in brackets e.g. <span className="text-slate-600 dark:text-slate-500">SetContactData (2)</span> = 2nd occurrence of that block type in this flow
        </span>
      </div>
      ...
      <div className="mt-6 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-slate-200/60 dark:bg-slate-800/60">
              <th className="text-left py-2.5 px-4 font-medium text-slate-500 dark:text-slate-400">#</th>
              <th className="text-left py-2.5 px-4 font-medium text-slate-500 dark:text-slate-400">Block</th>
              <th className="text-right py-2.5 px-4 font-medium text-slate-500 dark:text-slate-400">Contacts</th>
              <th className="text-right py-2.5 px-4 font-medium text-slate-500 dark:text-slate-400">% of Total</th>
              <th className="text-right py-2.5 px-4 font-medium text-slate-500 dark:text-slate-400">
                Exited after block
                <span className="ml-1 text-[9px] text-slate-600">(didn't reach next)</span>
              </th>
              <th className="text-right py-2.5 px-4 font-medium text-slate-500 dark:text-slate-400">Exit Type</th>
            </tr>
          </thead>
          <tbody>
            {data.blocks.map((b, i) => {
              ...
              return (
                <tr key={i} className="border-t border-slate-200 dark:border-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800/20 transition">
                  ...
                  <td className="py-2.5 px-4">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-slate-800 dark:text-slate-200">{b.block_name}</span>
                      <span
                  ...
                  <td className="py-2.5 px-4 text-right tabular-nums">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <span className="text-slate-700 dark:text-slate-300 w-10">{b.pct?.toFixed(1)}%</span>
                    </div>
                  </td>
                  <td className="py-2.5 px-4 text-right tabular-nums">
                    {b.drop_count > 0 ? (
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-500/15">
            <TrendingDown size={18} className="text-cyan-600 dark:text-cyan-400" />
          </div>
          <div>
            <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Flow Funnel Analysis</h1>
        ...
        <button
          ...
          onClick={load}
          disabled={loading}
          className="inline-flex items-center gap-1.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3 py-2 text-xs text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 transition disabled:opacity-50"
        >
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/60 dark:bg-slate-900/60 p-5">
        {loading && (
          <div className="flex h-64 items-center justify-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            ...
            Loading funnel for <span className="font-mono text-slate-800 dark:text-slate-200">{flowName}</span>…
          </div>
        )}
        ...
        {!loading && !error && data && (
          <>
            <div className="mb-4 flex items-center gap-3">
              <BarChart2 size={14} className="text-cyan-600 dark:text-cyan-400" />
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                Funnel Drop-off —{' '}
                <span className="font-mono text-cyan-600 dark:text-cyan-400">{flowName}</span>
              </p>
              {data.mock && (
                <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-[9px] text-amber-600 dark:text-amber-400">
                  sample data
                </span>
              )}
            </div>
