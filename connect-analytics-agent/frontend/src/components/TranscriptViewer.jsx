...
function AudioPlayer({ url }) {
  ...
  if (err) return (
    <div className="mt-3 rounded-2xl border border-rose-500/25 bg-rose-500/10 px-4 py-3 text-xs text-rose-600 dark:text-rose-300">
      Could not load recording. The presigned URL may have expired.
    </div>
  );
  return (
    <div className="mt-3 rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 p-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-1 flex-col gap-1">
          ...
          <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400"><span>{fmt(cur)}</span><span>{dur ? fmt(dur) : '--:--'}</span></div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-5 h-full">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="rounded-2xl bg-connect-500/15 p-2.5 text-connect-700 dark:text-connect-400">
            {isVoice ? <Phone size={16} /> : <MessageSquare size={16} />}
          </div>
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400">Contact</p>
            <h4 className="font-semibold text-sm text-slate-900 dark:text-white">{contact?.contactId || '—'}</h4>
          </div>
        </div>
        {onClose && (
          <button type="button" onClick={onClose}
            className="rounded-xl border border-slate-300 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400 hover:border-slate-500 hover:text-slate-800 dark:hover:text-slate-200">
            <X size={16} />
          </button>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        {contact?.queue && <span className="rounded-full border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3 py-1 text-slate-700 dark:text-slate-300">{contact.queue}</span>}
        {contact?.agent && <span className="rounded-full border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3 py-1 text-slate-700 dark:text-slate-300">{contact.agent}</span>}
        {contact?.status && <span className="rounded-full border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3 py-1 text-slate-700 dark:text-slate-300">{contact.status}</span>}
        {contact?.duration != null && <span className="rounded-full border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3 py-1 text-slate-700 dark:text-slate-300">{fmtDuration(contact.duration)}</span>}
        {channel && <span className={`rounded-full border px-3 py-1 ${isVoice ? 'border-connect-500/30 text-connect-700 dark:text-connect-400' : 'border-emerald-500/30 text-emerald-600 dark:text-emerald-300'}`}>{channel}</span>}
        {isEscalated && (
          <span className="flex items-center gap-1 rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1 text-amber-600 dark:text-amber-300">
            <Bot size={10} />Escalated from Bot
          </span>
        )}
        {isBotOnlyCompleted && (
          <span className="flex items-center gap-1 rounded-full border border-violet-500/30 bg-violet-500/10 px-3 py-1 text-violet-600 dark:text-violet-300">
      ...
      {!contact?.contactId && (
        <div className="flex gap-2">
          <div className="flex flex-1 items-center gap-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-2 text-sm">
            <Search size={14} className="text-slate-500 dark:text-slate-400" />
            <input value={manualId} onChange={(e) => setManualId(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && fetchT(manualId)}
              placeholder="Enter Contact ID…" className="flex-1 bg-transparent outline-none text-slate-900 dark:text-slate-100" />
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button type="button" onClick={handleSumm} disabled={summarizing}
          className="inline-flex items-center gap-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-4 py-2 text-sm text-slate-900 dark:text-slate-100 hover:border-connect-500 disabled:opacity-50">
      ...
      {recMsg && (
        <div className="flex items-start gap-2 rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-600 dark:text-amber-300">
      ...
      {(summarizing || summary || summErr) && (
        <div className="rounded-2xl border border-connect-500/30 bg-connect-500/10">
          <button type="button" onClick={() => setSummOpen(v => !v)}
            className="flex w-full items-center justify-between px-4 py-3 text-sm font-medium text-connect-700 dark:text-connect-400">
          ...
          {summOpen && (
            <div className="border-t border-connect-500/20 px-4 py-4">
              {summarizing && <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400"><RefreshCw size={13} className="animate-spin" />Generating…</div>}
              {summErr && <div className="flex items-start gap-2 text-sm text-rose-600 dark:text-rose-300"><AlertCircle size={13} className="mt-0.5 shrink-0" />{summErr}</div>}
              {summary && <div className="prose prose-sm dark:prose-invert max-w-none text-slate-800 dark:text-slate-200"><ReactMarkdown rehypePlugins={[rehypeSanitize]}>{summary}</ReactMarkdown></div>}
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto">
        {tErr && <div className="mb-3 flex items-center gap-2 rounded-xl border border-rose-500/25 bg-rose-500/10 px-4 py-3 text-sm text-rose-600 dark:text-rose-300"><AlertCircle size={14} />{tErr}</div>}
        ...
        {!tErr && tStatus === 'PROCESSING' && segments.length === 0 && (
          <div className="mb-3 rounded-xl border border-sky-500/25 bg-sky-500/10 px-4 py-4">
            ...
            {tMsg && <p className="mt-2 text-xs text-sky-600/70 dark:text-sky-300/70">{tMsg}</p>}
            {countdown !== null && countdown > 0 && (
              <div className="mt-3 flex items-center gap-2">
                ...
                <button
                  ...
                  className="rounded-lg border border-sky-500/30 px-2.5 py-1 text-xs text-sky-600 dark:text-sky-300 hover:bg-sky-500/20 transition-colors"
                >
                  Retry now
                </button>
              </div>
        ...
        {!tErr && tStatus !== 'PROCESSING' && tMsg && segments.length === 0 && (
          <div className={`mb-3 flex items-start gap-2 rounded-xl border px-4 py-3 text-sm ${
            tStatus === 'NOT_FOUND'
              ...
              : 'border-slate-300 dark:border-slate-700 bg-slate-200/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400'
          }`}>
        ...
        {loadingT ? (
          <div className="flex items-center justify-center gap-3 py-12 text-sm text-slate-500 dark:text-slate-400"><RefreshCw size={16} className="animate-spin" />Loading transcript…</div>
        ) : segments.length === 0 ? (
          !tMsg && !tErr && <p className="py-8 text-center text-sm text-slate-500 dark:text-slate-400">No transcript segments. Select a contact above.</p>
        ) : (
          <div className="space-y-3">
            {/* Bot interaction header for automated_interaction_log transcripts */}
            {transcriptSource === 'automated_interaction_log' && (
              <div className="flex items-center gap-2 rounded-xl border border-violet-500/25 bg-violet-500/10 px-4 py-2.5 text-xs text-violet-600 dark:text-violet-300">
            ...
            {segments.map((seg, i) => {
              ...
              return (
                <div key={i} className={`rounded-2xl border p-4 ${isAgent ? 'border-connect-500/25 bg-connect-500/10' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900'}`}>
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-900 dark:text-slate-100">{highlight(seg.text, highlightKeyword)}</p>
                </div>
              );
            })}
            ...
            {isEscalated && (
              <div className="relative flex items-center gap-3 py-2">
                ...
                <div className="flex shrink-0 items-center gap-1.5 rounded-full border border-amber-500/40 bg-amber-500/10 px-3 py-1 text-xs text-amber-600 dark:text-amber-300">
                  <ArrowDownToLine size={11} />
                  Escalated to {contact.queueName || contact.queue || 'agent queue'}
                  {contact.enqueuedAt && (
                    <span className="ml-1 text-amber-600/70 dark:text-amber-400/70">· {new Date(contact.enqueuedAt).toLocaleTimeString()}</span>
                  )}
                </div>
                <div className="h-px flex-1 bg-amber-500/30" />
              </div>
            ...
            {isEscalated && (
              <div className="flex items-center gap-2 rounded-xl border border-connect-500/25 bg-connect-500/10 px-4 py-2.5 text-xs text-connect-700 dark:text-connect-400">
                <UserCheck size={13} className="shrink-0" />
                {contact.connectedToAgentAt
                  ? <>Agent connected at {new Date(contact.connectedToAgentAt).toLocaleTimeString()}</>
                  : 'Waiting for agent…'}
                {contact.agent && <span className="ml-auto text-slate-500 dark:text-slate-400">{contact.agent}</span>}
              </div>
            )}
          </div>
        )}
...
function CallTile({ call, colourClass, onClick }) {
  ...
  return (
    <button type="button" onClick={() => onClick(call)}
      ...
      {/* Live badge */}
      <span className="absolute top-3 right-3 flex items-center gap-1.5 rounded-full bg-rose-500/20 px-2 py-0.5 text-[10px] font-medium text-rose-600 dark:text-rose-300">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-2">
        ...
        {call.escalatedFromBot && (
          <span className="flex items-center gap-1 rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[10px] text-amber-600 dark:text-amber-300">
            <Bot size={9} />🤖→👤
          </span>
        )}
      </div>
      <p className="font-semibold text-sm text-slate-900 dark:text-slate-100 truncate pr-10">{call.agent || 'Unknown agent'}</p>
      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">{call.contactId}</p>
      <p className="mt-3 font-mono text-lg font-semibold text-slate-900 dark:text-white">{fmtDuration(elapsed)}</p>
      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 truncate">{call.queue || call.queueName}</p>
      {call.escalatedFromBot && call.connectedToAgentAt && (
        <p className="mt-1 flex items-center gap-1 text-[10px] text-amber-600/70 dark:text-amber-300/70">
...
function Modal({ call, onClose, highlightKeyword }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex h-[90vh] w-full max-w-3xl flex-col rounded-3xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-2xl overflow-hidden">
...
function BotContactRow({ c, onClick }) {
  return (
    <button type="button" onClick={() => onClick(c)}
      className="flex w-full items-start gap-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3 text-left hover:border-slate-300 dark:hover:border-slate-600 transition">
      <div className={`mt-0.5 rounded-xl p-2 ${c.escalatedToAgent ? 'bg-amber-500/15 text-amber-600 dark:text-amber-300' : 'bg-violet-500/15 text-violet-600 dark:text-violet-300'}`}>
        <Bot size={14} />
      </div>
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
          <span className="font-mono text-slate-700 dark:text-slate-300 truncate">{c.contactId}</span>
          {c.escalatedToAgent && (
            <span className="flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-0.5 text-amber-600 dark:text-amber-300">
              <ArrowDownToLine size={9} />Escalated → {c.queueName}
            </span>
          )}
          {!c.escalatedToAgent && (
            <span className="rounded-full bg-violet-500/15 px-2 py-0.5 text-violet-600 dark:text-violet-300">Bot only</span>
          )}
          {c.inProgress && <span className="flex items-center gap-1 text-rose-600 dark:text-rose-300"><span className="h-1 w-1 animate-pulse rounded-full bg-rose-400" />in progress</span>}
        </div>
        <div className="flex flex-wrap gap-3 text-[11px] text-slate-500 dark:text-slate-400">
          ...
          {c.connectedToAgentAt && <span className="text-connect-700 dark:text-connect-400">Agent {new Date(c.connectedToAgentAt).toLocaleTimeString()}</span>}
        </div>
      </div>
    </button>
  );
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 p-5 shadow-panel">
        <button type="button" className="flex w-full items-center justify-between" onClick={() => setIntegrOpen(v => !v)}>
          <div className="flex items-center gap-2">
            ...
            {streams.connected && (
              <span className="flex items-center gap-1 rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-xs text-emerald-600 dark:text-emerald-300">
                <Wifi size={10} />Streams connected
              </span>
            )}
            {ebActive && (
              <span className="flex items-center gap-1 rounded-full bg-sky-500/15 px-2.5 py-0.5 text-xs text-sky-600 dark:text-sky-300">
                <Zap size={10} />EventBridge active
              </span>
            )}
            {liveBotCount > 0 && (
              <span className="flex items-center gap-1 rounded-full bg-violet-500/15 px-2.5 py-0.5 text-xs text-violet-600 dark:text-violet-300">
                <Bot size={10} />{liveBotCount} bot{liveBotCount !== 1 ? 's' : ''} live
              </span>
            )}
          </div>
          {integrOpen ? <ChevronUp size={16} className="text-slate-500 dark:text-slate-400" /> : <ChevronDown size={16} className="text-slate-500 dark:text-slate-400" />}
        </button>

        {integrOpen && (
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {/* ── Connect Streams card ── */}
            <div className="rounded-2xl border border-slate-300 dark:border-slate-700 bg-white/60 dark:bg-slate-950/60 p-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <div className={`rounded-xl p-2 ${streams.connected ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-300' : 'bg-slate-200/50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400'}`}>
                    {streams.connected ? <Wifi size={15} /> : <WifiOff size={15} />}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">Connect Streams SDK</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">Browser real-time contact events</p>
                  </div>
                </div>
                {streams.connected ? (
                  <button type="button" onClick={streams.disconnect}
                    className="rounded-xl border border-slate-300 dark:border-slate-600 px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 hover:border-rose-500 hover:text-rose-600 dark:hover:text-rose-300">
                    Disconnect
                  </button>
                ) : (
              ...
              {streams.error && (
                <div className="mt-3 flex items-start gap-2 rounded-xl border border-rose-500/25 bg-rose-500/10 px-3 py-2 text-xs text-rose-600 dark:text-rose-300">
                  <AlertCircle size={12} className="mt-0.5 shrink-0" />{streams.error}
                </div>
              )}

              {!streams.ccpAvailable && (
                <div className="mt-3 rounded-xl border border-amber-500/20 bg-amber-500/8 px-3 py-2 text-xs text-amber-600 dark:text-amber-300">
                  <p className="font-medium">Setup required:</p>
                  <ol className="mt-1 list-decimal list-inside space-y-1 text-amber-600/80 dark:text-amber-300/80">
              ...
              {streams.connected && (
                <div className="mt-3 space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <span className="text-slate-800 dark:text-slate-200">{streams.agentState || '—'}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                    <span>Contacts visible</span>
                    <span className="text-slate-800 dark:text-slate-200">{streams.contacts.length}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                    <span>Bot/IVR contacts</span>
                    <span className={streamsBotContacts.length > 0 ? 'text-violet-600 dark:text-violet-300' : 'text-slate-800 dark:text-slate-200'}>
            ...
            <div className="rounded-2xl border border-slate-300 dark:border-slate-700 bg-white/60 dark:bg-slate-950/60 p-4">
              <div className="flex items-center gap-2 mb-3">
                <div className={`rounded-xl p-2 ${ebActive ? 'bg-sky-500/15 text-sky-600 dark:text-sky-300' : 'bg-slate-200/50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400'}`}>
                  <Zap size={15} />
                </div>
                <div>
                  <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">EventBridge Pipeline</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Server-side contact event stream via SQS</p>
                </div>
              </div>

              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 dark:text-slate-400">SQS listener</span>
                  <span className={ebActive ? 'text-emerald-600 dark:text-emerald-300' : 'text-amber-600 dark:text-amber-300'}>
                    {ebActive ? '✓ Running' : '○ Not started'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Queue configured</span>
                  <span className={streamsConfig?.bot_events_queue_configured ? 'text-emerald-600 dark:text-emerald-300' : 'text-amber-600 dark:text-amber-300'}>
                    {streamsConfig?.bot_events_queue_configured ? '✓ Yes' : '○ Not set'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Live bot contacts</span>
                  <span className={ebContacts.length > 0 ? 'text-violet-600 dark:text-violet-300 font-medium' : 'text-slate-700 dark:text-slate-300'}>
                    {ebContacts.length}
                  </span>
                </div>
              </div>
              ...
              {!ebActive && (
                <div className="mt-3 rounded-xl border border-sky-500/20 bg-sky-500/8 px-3 py-2 text-xs text-sky-600 dark:text-sky-300">
                  <p className="font-medium">Setup required:</p>
                  <ol className="mt-1 list-decimal list-inside space-y-1 text-sky-600/80 dark:text-sky-300/80">
                ...
                <div className="mt-3 space-y-1.5">
                  {ebContacts.slice(0, 3).map(c => (
                    <button key={c.contactId} type="button" onClick={() => setSelectedCall(c)}
                      className="flex w-full items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/60 dark:bg-slate-900/60 px-3 py-2 text-left hover:border-violet-500/30 transition">
                      <Bot size={11} className="shrink-0 text-violet-600 dark:text-violet-300" />
                      <span className="font-mono text-[10px] text-slate-700 dark:text-slate-300 truncate flex-1">{c.contactId}</span>
                      <span className={`rounded-full px-1.5 py-0.5 text-[9px] ${c.escalatedToAgent ? 'bg-amber-500/15 text-amber-600 dark:text-amber-300' : 'bg-violet-500/15 text-violet-600 dark:text-violet-300'}`}>
                        {c.contactState}
                      </span>
                    </button>
                  ))}
                  {ebContacts.length > 3 && (
                    <p className="text-[10px] text-center text-slate-500 dark:text-slate-400">+{ebContacts.length - 3} more below</p>
                  )}
                </div>
              )}
            </div>
        ...
        {streams.connected && streamsBotContacts.length > 0 && (
          <div className="mt-4">
            <p className="mb-2 flex items-center gap-2 text-xs font-medium text-violet-600 dark:text-violet-300">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {streamsBotContacts.map((c) => (
                <button key={c.contactId} type="button" onClick={() => setSelectedCall(c)}
                  ...
                  <span className="absolute top-3 right-3 flex items-center gap-1 rounded-full bg-violet-500/20 px-2 py-0.5 text-[10px] text-violet-600 dark:text-violet-300">
                  ...
                  <p className="font-mono text-xs text-slate-700 dark:text-slate-300 truncate pr-12">{c.contactId}</p>
                  <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">Handling by conversational AI bot</p>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={fetchCalls}
              className="inline-flex items-center gap-2 rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 hover:border-connect-500">
        ...
        <div className="mt-6">
          {loadingCalls ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-slate-500 dark:text-slate-400">
              ...
              {!streams.connected && <p className="text-xs text-slate-600 dark:text-slate-500">Connect to CCP above to also see bot-only calls</p>}
            </div>
          ) : (
              ...
              {queues.map((queue) => (
                <div key={queue}>
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">{queue}</h4>
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 p-5 shadow-panel">
        <div className="flex w-full items-center justify-between">
          ...
          >
            ...
            <span className="rounded-full bg-violet-500/15 px-2.5 py-0.5 text-xs text-violet-600 dark:text-violet-300">last 2 h · completed</span>
            {!loadingBot && botTotal > 0 && (
              <span className="rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 text-xs text-slate-700 dark:text-slate-300">{botTotal} total</span>
            )}
            {!loadingBot && escalatedCount > 0 && (
              <span className="rounded-full bg-amber-500/15 px-2.5 py-0.5 text-xs text-amber-600 dark:text-amber-300">
          ...
          <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
            <button type="button" onClick={() => fetchBotContacts()}
              className="rounded-xl border border-slate-300 dark:border-slate-700 p-1.5 hover:border-slate-500">
