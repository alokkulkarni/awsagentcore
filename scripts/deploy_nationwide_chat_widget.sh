...
# =============================================================================
# Deploys the nationwide_chat_widget React application to AWS CloudFront + S3.
#
# Architecture:
...
#
# Usage:
#   chmod +x scripts/deploy_nationwide_chat_widget.sh
#   ./scripts/deploy_nationwide_chat_widget.sh [OPTIONS]
#
# Options:
...
#   --bucket-name   NAME     S3 bucket name override (default: nationwide-connect-widget-{env})
...
#   - The nationwide_chat_widget/ directory must exist relative to the repo root
# =============================================================================
...
WIDGET_DIR="${REPO_ROOT}/nationwide_chat_widget"
...
SKIP_BUILD=false
DRY_RUN=false
PROJECT="nationwide-connect-widget"
...
CF_COMMENT="Nationwide Building Society Connect Widget (${ENV})"
OAC_NAME="${PROJECT}-oac-${ENV}"
HEADERS_POLICY_NAME="${PROJECT}-headers-${ENV}"
CF_TAGS="Key=Project,Value=nationwide Key=Environment,Value=${ENV} Key=ManagedBy,Value=deploy_nationwide_chat_widget"
...
save_state() {
    cat > "$STATE_FILE" <<EOF
# Auto-generated by deploy_nationwide_chat_widget.sh — do not edit manually
    ...
    cat > "$HEADERS_POLICY_CONFIG_FILE" <<JSON
{
    ...
    "Comment": "Security headers for Nationwide Building Society Connect Widget (${ENV})",
...
$(printf "${YELLOW}")⚠  If the distribution is still deploying (status: In Progress):$(printf "${NC}")
  ...
  To redeploy after code changes:
    ./scripts/deploy_nationwide_chat_widget.sh --env ${ENV} --region ${AWS_REGION}
